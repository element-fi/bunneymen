...
export function sigDataset(node: Node, account: string) {
  return Dataset.create<SignedData, any>(
    ...
    Loader.create({
      transformer: async (data) => {
        ...
        const provider = (await detectEthereumProvider()) as MinimumProvider

        if (!account || !provider || !('request' in provider)) {
